...
雙擊 `公開離線啟動.bat` 或 `開機自動啟動-公開離線.bat`。服務會在可見的終端機執行，視窗顯示可分享的 HTTPS 網址與存取紀錄。關閉該終端機或按 `Ctrl+C`，伺服器就會停止。正常停止時腳本也會關閉 PowerQuery 的 8443 Funnel；若直接關閉視窗導致清理程序無法執行，Funnel 設定可能暫時保留，但本機伺服器已停止。
...
2. 對 `開機自動啟動-公開離線.bat` 建立捷徑，將捷徑的視窗執行方式設為「一般視窗」。
3. 把捷徑放入開啟的「啟動」資料夾；不要複製批次檔本身。登入 Windows 後會顯示終端機，關閉它就會停止伺服器。
